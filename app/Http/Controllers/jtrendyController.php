...
class jtrendyController extends Controller
{
    ...
    if($request->hasFile('myVideo')){
       if(file_exists(public_path('videos/'.$oldvideo))){
        ...
        return redirect()->route('songList')->with( 'message','Song was Updated!'); 
       
    }

    public function jsongList()
    {
        $jsongListCompact=DB::table('song')->orderBy('song_react_count','desc')->paginate(5);
    ...
    public function userCreate(Request $request){
        ...
        $phone=$request->phone_number%100000000;
